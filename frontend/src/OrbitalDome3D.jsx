    ...
    <group>
      ...
      >
        ...
        {isSatellite && (isHovered || obj.name.toUpperCase().includes("ISS") || obj.name.toUpperCase().includes("TIANGONG") || obj.name.toUpperCase().includes("CSS")) && (
          <Html distanceFactor={4} position={[0, 0.2, 0]} pointerEvents="none">
            <div className="flex flex-col items-center font-mono select-none">
              <div 
                className="px-1.5 py-0.5 text-[8px] font-black tracking-widest bg-black/80 border rounded-sm whitespace-nowrap"
              ...
              {isHovered && (
                <div 
                  className="mt-1 p-1.5 bg-[#060a12]/95 border text-[7px] leading-tight space-y-0.5 rounded-sm shadow-lg shadow-black w-24 text-gray-300"
                  style={{ borderColor: fallbackColor }}
                >
                  <div className="font-black text-white border-b border-cyan-500/20 pb-0.5 mb-0.5">TELEMETRY LOCK</div>
